...
describe('Query API E2E', () => {
  ...
  beforeAll(async () => {
    ...
    const fixtures: Array<{ url: string; vendor: 'trackpro' | 'swingmetric' | 'proswing'; fixture: string }> = [
      { url: '/v1/webhooks/trackpro',     vendor: 'trackpro',     fixture: 'trackpro.retransmit.json' },
      { url: '/v1/webhooks/swingmetric',  vendor: 'swingmetric',  fixture: 'swingmetric.cross-batch-retransmit.json' },
      { url: '/v1/webhooks/proswing',     vendor: 'proswing',     fixture: 'proswing.tz-offset.json' },
    ];
    ...
    for (const { url, vendor, fixture } of fixtures) {
      await request(app.getHttpServer())
        ...
        .send(freshenFixture(loadFixture(fixture) as Record<string, unknown>, vendor));
    }
  ...
  it('GET /v1/users/by-vendor/trackpro/:id/shots → 200 with data array', async () => {
    const payload = loadFixture('trackpro.retransmit.json') as Record<string, unknown>;
  ...
  it('GET /v1/users/by-vendor/:vendor/:id/shots with club filter → filtered results', async () => {
    ...
    for (const shot of resFiltered.body.data as Array<Record<string, unknown>>) {
