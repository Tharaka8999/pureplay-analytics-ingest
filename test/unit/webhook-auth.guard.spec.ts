...
function buildGuard(
  ...
  return new WebhookAuthGuard(config as never, reflector as never, logger as never);
}
...
describe('WebhookAuthGuard [SEC]', () => {
  ...
  describe('mode=api_key', () => {
    ...
    it('[SEC] rejects vendor A key on vendor B endpoint (cross-vendor rejection)', async () => {
      ...
      const swingmetricGuard = buildGuard('api_key', 'sm-secret', 'swingmetric');
      const ctx = makeContext({ 'x-webhook-auth': 'tp-secret' }); // wrong key for SM
      await expect(swingmetricGuard.canActivate(ctx)).rejects.toThrow(UnauthorizedException);
    });
  ...
  describe('mode=hmac', () => {
    ...
    const rawBody = Buffer.from(JSON.stringify({ event: 'shot.captured', id: 'tp_001' }));
    ...
    it('rejects when X-Webhook-Signature header is missing', async () => {
      const guard = buildGuard('hmac', 'unused', 'trackpro', HMAC_SECRET);
      const ts = String(Math.floor(Date.now() / 1000));
      const ctx = makeContext({ 'x-webhook-timestamp': ts }, 'trackpro', rawBody);
      await expect(guard.canActivate(ctx)).rejects.toThrow(UnauthorizedException);
    });
    ...
    it('[SEC] rejects cross-vendor key misuse in hmac mode', async () => {
      ...
      const guard = buildGuard('hmac', 'unused', 'swingmetric', SWINGMETRIC_SECRET);
      const ts = String(Math.floor(Date.now() / 1000));
      const sigWithTrackproSecret = signPayload('trackpro-wrong-secret', ts, rawBody);
      const ctx = makeContext(
        { 'x-webhook-timestamp': ts, 'x-webhook-signature': sigWithTrackproSecret },
        'swingmetric',
        rawBody,
      );
      await expect(guard.canActivate(ctx)).rejects.toThrow(UnauthorizedException);
