...
async function bootstrap(): Promise<void> {
  ...
  fastify.addHook('onRequest', async (_request: unknown, reply: { header: (k: string, v: string) => void }) => {
    ...
    void reply.header(
      ...
      'Content-Type,Authorization,X-Correlation-ID,X-Webhook-Auth,X-Webhook-Timestamp,X-Webhook-Signature',
    );
  });
  fastify.addHook('onRequest', async (request: { method: string }, reply: { status: (s: number) => { send: () => void } }) => {
  ...
  await (fastify as { register: (p: unknown, o?: unknown) => Promise<void> }).register(compress, { global: true });
  await (fastify as { register: (p: unknown, o?: unknown) => Promise<void> }).register(helmet, {
