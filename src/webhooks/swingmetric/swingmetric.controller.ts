...
export class SwingmetricController {
  ...
  @ApiResponse({
    status: 422,
    description:
      "One or more shots rejected — captured_at is outside the ingestion window",
    schema: {
      example: {
  ...
  async receive(
    ...
    const skewedShot = shots.find((s) =>
      hasExcessiveClockSkew(s.captured_at_utc, receivedAtUtc),
    );
    if (skewedShot) {
    ...
    await Promise.all(
      shots.map((shot) =>
        this.queue.enqueue(shot, correlationId, receivedAtUtc),
      ),
    );
