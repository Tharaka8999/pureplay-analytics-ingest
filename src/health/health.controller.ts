...
export class HealthController {
  ...
  @Get('healthz')
  @ApiOperation({ summary: 'Liveness probe', description: 'Returns 200 immediately if the process is alive. No external dependency checks.' })
  @ApiResponse({ status: 200, schema: { example: { status: 'ok', timestamp: '2026-05-19T04:00:00.000Z' } } })
  ...
  @Get('readyz')
  @HealthCheck()
  @ApiOperation({ summary: 'Readiness probe', description: 'Checks Postgres + Redis connectivity. Returns 503 if either is down.' })
    ...
    return this.health.check([
      async (): Promise<HealthIndicatorResult> => {
        try {
          ...
          return Promise.reject({ db: { status: 'down', error: 'db_unavailable' } });
        }
      },
      async (): Promise<HealthIndicatorResult> => {
        try {
          ...
          return Promise.reject({ redis: { status: 'down', error: 'redis_unavailable' } });
        }
      },
    ]);
  }
