...
export function getShotsTotal(): Counter {
  if (!_shotsTotal) {
    try {
      ...
      _shotsTotal = register.getSingleMetric('pureplay_ingest_shots_total') as Counter;
    }
  }
  return _shotsTotal;
}
...
export function getE2eLag(): Histogram {
  if (!_e2eLag) {
    try {
      ...
      _e2eLag = register.getSingleMetric('pureplay_ingest_e2e_lag_ms') as Histogram;
    }
  }
  return _e2eLag;
}
...
export function getQueueDepth(): Gauge {
  if (!_queueDepth) {
    try {
      ...
      _queueDepth = register.getSingleMetric('pureplay_ingest_queue_depth') as Gauge;
    }
  }
  return _queueDepth;
}
...
export function getJobsFailed(): Counter {
  if (!_jobsFailed) {
    try {
      ...
      _jobsFailed = register.getSingleMetric('pureplay_ingest_jobs_failed_total') as Counter;
    }
  }
  return _jobsFailed;
}
