...
export class GlobalExceptionFilter implements ExceptionFilter {
  private readonly logger = new Logger(GlobalExceptionFilter.name);
    ...
    const correlationId =
      (request.headers as Record<string, string | undefined>)['x-correlation-id'] ??
      request.id ??
      'unknown';
    ...
    if (exception instanceof DomainError) {
      ...
      if (status >= 500) {
        this.logger.error({ correlation_id: correlationId, err: exception }, exception.message);
      } else {
        this.logger.warn({ correlation_id: correlationId, error_code: errorCode }, exception.message);
      }
    ...
    if (exception instanceof HttpException) {
      ...
      const retryAfter = (typeof exceptionResponse === 'object' && exceptionResponse !== null)
        ? (exceptionResponse as Record<string, unknown>)['retryAfter'] as number | undefined
        : undefined;

      if (status >= 500) {
        this.logger.error({ correlation_id: correlationId, err: exception }, 'HTTP error');
      } else {
        this.logger.warn({ correlation_id: correlationId, error_code: errorCode }, title);
      }
      ...
      const body = buildProblem(status, errorCode, title, correlationId, issues);
      void reply.status(status).send(body);
      return;
    }

    // ── Unhandled exception — never expose internals ──────────────────────────
    this.logger.error({ correlation_id: correlationId, err: exception }, 'Unhandled exception');
    void reply.status(HttpStatus.INTERNAL_SERVER_ERROR).send(
      buildProblem(500, 'INTERNAL_ERROR', 'An unexpected error occurred.', correlationId),
    );
  }
}
...
function mapStatusToCode(status: number): string {
  switch (status) {
    case 400: return 'PAYLOAD_VALIDATION_FAILED';
