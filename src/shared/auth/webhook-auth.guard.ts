...
export class WebhookAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const mode = this.config.get('WEBHOOK_AUTH_MODE', { infer: true }) ?? 'none';
    ...
    if (mode === 'hmac') {
      ...
      if (!vendor) {
        throw new UnauthorizedException('No vendor metadata configured for this route.');
      }

      const envSecretKey = `${vendor.toUpperCase()}_HMAC_SECRET` as keyof Env;
      const secret = this.config.get(envSecretKey, { infer: true }) as string | undefined;
      if (!secret) {
        throw new UnauthorizedException('HMAC secret not configured for this vendor.');
      }
      ...
      if (!tsHeader || !sigHeader) {
        throw new UnauthorizedException('Missing X-Webhook-Timestamp or X-Webhook-Signature header.');
      }
      ...
      if (isNaN(tsSec) || Math.abs(nowSec - tsSec) > 300) {
        throw new UnauthorizedException('Request timestamp is outside the 5-minute replay window.');
      }
    ...
    const expectedKey = this.config.get(envKey, { infer: true }) as string | undefined;

    const providedKey = (req.headers as Record<string, string | undefined>)['x-webhook-auth'];

    if (!providedKey || !expectedKey) {
      throw new UnauthorizedException('Missing or unconfigured webhook authentication key.');
    }
