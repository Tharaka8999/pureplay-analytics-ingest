...
export class IdentityController {
  ...
  @ApiHeader({
    ...
    description: "UUID; safe to retry with same key for 24h",
    required: false,
  })
  @ApiParam({
    name: "canonical_user_id",
    description: "Canonical user ULID (26 chars)",
  })
  @ApiBody({
    schema: {
  ...
  @ApiResponse({
    status: 201,
    description: "Identity linked; existing shots backfilled",
    headers: {
      Location: {
        description: "/v1/users/:canonical_user_id/identities",
        schema: { type: "string" },
      },
    },
  })
  ...
  @ApiOperation({
    ...
    description:
      "Returns all (vendor, vendor_user_id) pairs linked to this canonical user.",
  })
  ...
  @ApiParam({
    name: "vendor",
    description: "trackpro | swingmetric | proswing",
  })
  @ApiParam({
    name: "vendor_user_id",
    description: "Vendor-scoped user identifier",
  })
  ...
  async unlinkIdentity(
    ...
    await this.identityService.unlinkIdentity(
      vendor as Vendor,
      vendorUserId,
      canonicalUserId,
    );
